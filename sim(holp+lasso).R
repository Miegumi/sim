...
sim1 = function(n,p,p1,r.square){
  ...
  test_pre_holp<- predict(cv.fit_holp,newx=x_holp_test,s=cv.fit_holp$lambda.min)
    ...
    return(list(mse,mse_holp))
}

result_mse= foreach(i=1:10, .combine = "rbind") %do% sim1(n,p,p1,r.square)$mse      ##rerun 100 times
result_mse_holp=foreach(i=1:10, .combine = "rbind") %do% sim1(n,p,p1,r.square)$mse_holp
sum(result_mse)/10
sum(result_mse_holp)/10
